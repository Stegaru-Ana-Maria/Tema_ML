...
for subdir in subdirectories:
    ...
    train_correct_predictions = sum(predict(decision_tree, X_train[i]) == y_train[i] for i in range(len(X_train)))
    train_accuracy = train_correct_predictions / len(X_train)
    train_accuracies.append(train_accuracy)

    test_correct_predictions = sum(predict(decision_tree, X_test[i]) == y_test[i] for i in range(len(X_test)))
...
plt.title('Acuratete pe Setul de Antrenare și Testare pentru Arborele de Decizie')
